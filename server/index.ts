...
type MediaProfile = {
  duration: number; size: number; width?: number; height?: number; frameRate?: number; bitRate?: number; videoCodec?: string; audioCodec?: string;
  pixelFormat?: string; colorTransfer?: string; colorPrimaries?: string; colorSpace?: string; hdr: boolean;
};
type Project = {
  id: string; sourcePath: string; workDir: string; sourceName: string; proxyPath?: string; audioPath?: string;
  transcript?: { text: string; words: Word[] }; edl?: { keepRanges: KeepRange[]; notes?: string[] }; media: MediaProfile;
};
type LocalSettings = {
...
async function pickNativeFile() {
  if (process.platform === 'darwin') { const { stdout } = await run('osascript', ['-e', 'POSIX path of (choose file with prompt "Choose a talking-head video")']); return stdout.trim(); }
  ...
  const { stdout } = await run('zenity', ['--file-selection', '--title=Choose a talking-head video', '--file-filter=Video | *.mov *.mp4 *.m4v *.webm']); return stdout.trim();
}
...
function getProject(id: string) { const project = projects.get(id); if (!project) throw new Error('Project not found. Re-select the source video after restarting the local server.'); return project; }
async function saveProject(project: Project) { await fs.writeFile(path.join(project.workDir, 'project.json'), JSON.stringify(project, null, 2)); }
function serializeProject(project: Project) { return { id: project.id, sourceName: project.sourceName, media: project.media }; }
async function invalidateBroll(project: Project) { brollPlans.delete(project.id); await fs.rm(path.join(project.workDir, 'broll-plan.json'), { force: true }); }
...
async function ensureAnalysisAudio(project: Project) {
  ...
  await run(settings.ffmpegBin, ['-hide_banner', '-y', '-i', project.sourcePath, '-map', '0:a:0', '-vn', '-ac', '1', '-ar', '16000', '-c:a', 'aac', '-b:a', '48k', audioPath]); project.audioPath = audioPath; await saveProject(project); return audioPath;
}
async function transcribeProject(project: Project) {
  ...
  await Promise.all([fs.writeFile(path.join(project.workDir, 'transcript.json'), JSON.stringify(project.transcript, null, 2)), saveProject(project)]); return project.transcript;
}
...
function encodingArgs(project: Project, capabilities: FfmpegCapabilities, mode: 'fast' | 'quality') {
  const sourceCodec = String(project.media.videoCodec || '').toLowerCase(); const useHevc = project.media.hdr || sourceCodec === 'hevc' || sourceCodec === 'h265'; const hardware = useHevc ? capabilities.hevcVideoToolbox : capabilities.h264VideoToolbox; const targetBitRate = exportBitRate(project.media, useHevc, mode);
  if (hardware) { const encoder = useHevc ? 'hevc_videotoolbox' : 'h264_videotoolbox'; return { encoder, hardware, targetBitRate, args: ['-c:v', encoder, '-realtime', '1', '-allow_sw', '1', ...(mode === 'fast' ? ['-prio_speed', '1'] : []), '-b:v', formatBitRate(targetBitRate), '-maxrate', formatBitRate(Math.round(targetBitRate * 1.2)), '-bufsize', formatBitRate(Math.round(targetBitRate * 2)), ...(useHevc ? ['-tag:v', 'hvc1'] : []), ...(project.media.hdr && useHevc ? ['-profile:v', 'main10'] : [])] }; }
  if (useHevc) return { encoder: 'libx265', hardware: false, targetBitRate, args: ['-c:v', 'libx265', '-preset', mode === 'fast' ? 'veryfast' : 'fast', '-crf', mode === 'fast' ? '19' : '16', ...(project.media.hdr ? ['-pix_fmt', 'yuv420p10le'] : []), '-tag:v', 'hvc1'] };
...
app.put('/api/settings', route(async (req, res) => {
  const body = req.body ?? {}; if (typeof body.elevenLabsApiKey === 'string' && body.elevenLabsApiKey.trim()) localSettings.elevenLabsApiKey = body.elevenLabsApiKey.trim(); if (typeof body.openAiApiKey === 'string' && body.openAiApiKey.trim()) localSettings.openAiApiKey = body.openAiApiKey.trim(); if (typeof body.geminiApiKey === 'string' && body.geminiApiKey.trim()) localSettings.geminiApiKey = body.geminiApiKey.trim(); if (typeof body.imageProvider === 'string') localSettings.imageProvider = providerValue(body.imageProvider);
  ...
  capabilityCache.clear(); await saveSettings(); res.json(await systemStatus());
}));

app.post('/api/projects/select', route(async (_req, res) => { const sourcePath = await pickNativeFile(); if (!sourcePath) return void res.status(400).json({ error: 'No video selected' }); const settings = await resolvedSettings(); const id = randomUUID(); const workDir = path.join(settings.projectsDir, id); await fs.mkdir(workDir, { recursive: true }); const project: Project = { id, sourcePath, sourceName: path.basename(sourcePath), workDir, media: await probe(sourcePath) }; projects.set(id, project); await saveProject(project); res.json(serializeProject(project)); }));
app.post('/api/projects/:id/prepare', route(async (req, res) => {
  const project = getProject(routeParam(req.params.id)); const settings = await resolvedSettings(); if (!settings.ffmpegBin) throw new Error('FFmpeg was not found.'); const capabilities = await ffmpegCapabilities(settings.ffmpegBin); const audioPath = path.join(project.workDir, 'analysis.m4a'); const proxyPath = path.join(project.workDir, 'proxy.mp4'); const dimensions = proxySize(project.media); const inputAcceleration = capabilities.videoToolboxDecode ? ['-hwaccel', 'videotoolbox'] : []; const proxyEncoder = capabilities.h264VideoToolbox ? ['-c:v', 'h264_videotoolbox', '-realtime', '1', '-prio_speed', '1', '-allow_sw', '1', '-b:v', '1500k', '-maxrate', '2500k', '-bufsize', '4M'] : ['-c:v', 'libx264', '-preset', 'veryfast', '-crf', '28'];
  await run(settings.ffmpegBin, ['-hide_banner', '-y', ...inputAcceleration, '-i', project.sourcePath, '-map', '0:v:0', '-map', '0:a:0', '-vf', `scale=${dimensions.width}:${dimensions.height}:flags=fast_bilinear,fps=30,format=yuv420p`, ...proxyEncoder, '-c:a', 'aac', '-b:a', '96k', '-movflags', '+faststart', proxyPath, '-map', '0:a:0', '-vn', '-ac', '1', '-ar', '16000', '-c:a', 'aac', '-b:a', '48k', audioPath]); project.audioPath = audioPath; project.proxyPath = proxyPath; await saveProject(project); res.json({ proxyUrl: `/api/projects/${project.id}/proxy`, proxy: { width: dimensions.width, height: dimensions.height, fps: 30, hardware: capabilities.h264VideoToolbox } });
}));
...
app.post('/api/projects/:id/broll/scenes/:sceneId/video', route(async (req, res) => {
  ...
  scene = await generateBrollVideoWithGrokCli({ workDir: project.workDir, plan, sceneId, config: { grokBin: settings.grokBin, grokModel: settings.grokModel, grokVideoModel: settings.grokVideoModel, codexBin: settings.codexBin, ffmpegBin: settings.ffmpegBin } }); brollPlans.set(project.id, plan); res.json({ scene, videoUrl: `/api/projects/${project.id}/broll/scenes/${scene.id}/video?v=${encodeURIComponent(scene.videoGeneratedAt ?? '')}` });
}));
...
const PORT = Number(process.env.PORT || 3001); await loadSettings();
...
app.listen(PORT, '127.0.0.1', () => { console.log(`Video Cleaner local server: http://127.0.0.1:${PORT}`); });
