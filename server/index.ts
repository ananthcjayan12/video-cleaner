...
app.post('/api/projects/:id/broll/scenes/:sceneId/video', route(async (req, res) => {
  ...
  scene = await generateBrollVideoWithGrokCli({ workDir: project.workDir, plan, sceneId, config: { grokBin: settings.grokBin, grokModel: settings.grokModel, grokVideoModel: settings.grokVideoModel, codexBin: settings.codexBin, ffmpegBin: settings.ffmpegBin } }); brollPlans.set(project.id, plan); await touchProject(project); res.json({ scene, videoUrl: `/api/projects/${project.id}/broll/scenes/${scene.id}/video?v=${encodeURIComponent(scene.videoGeneratedAt ?? '')}` });
}));
