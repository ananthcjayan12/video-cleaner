...
async function pickNativeFile() {
  ...
  try {
    ...
    throw new Error('No supported native file picker found. Install zenity or set up the project on macOS/Windows.');
  }
}

async function pickExportPath() {
  if (process.platform === 'darwin') {
    const { stdout } = await run('osascript', ['-e', 'POSIX path of (choose file name with prompt "Export cleaned video" default name "cleaned-video.mp4")']);
    return stdout.trim();
  }
  if (process.platform === 'win32') {
    const script = [
  ...
  try {
    const { stdout } = await run('zenity', ['--file-selection', '--save', '--confirm-overwrite', '--filename=cleaned-video.mp4']);
    return stdout.trim();
  } catch {
    throw new Error('No supported native save dialog found.');
  }
}
...
app.get('/api/system/status', route(async (_req, res) => {
...
app.get('/api/settings', route(async (_req, res) => {
  ...
  res.json({
    ...status,
    overrides: {
      codexBin: localSettings.codexBin ?? '',
      ffmpegBin: localSettings.ffmpegBin ?? '',
      ffprobeBin: localSettings.ffprobeBin ?? '',
      projectsDir: localSettings.projectsDir ?? '',
    },
  });
}));
...
app.post('/api/projects/:id/transcribe', route(async (req, res) => {
  ...
  res.json({ transcript: project.transcript, edl: project.edl });
}));
...
app.post('/api/projects/:id/clean', route(async (req, res) => {
  ...
  if (!settings.codexBin) throw new Error('Codex CLI was not found. Configure CODEX_BIN or install Codex.');
  ...
  const transcript = project.transcript.words
