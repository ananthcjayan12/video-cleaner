...
async function renderBrollScenePreview(project: Project, plan: BrollPlan, sceneId: string, ffmpegBin: string) {
  ...
  const cacheKey = createHash('sha256').update(JSON.stringify({ previewPipeline: 3, sourcePath, sourceSize: sourceStat.size, sourceMtime: sourceStat.mtimeMs, start: sourceStart, assetPath, assetSize: assetStat.size, assetMtime: assetStat.mtimeMs, duration, template, width: dimensions.width, height: dimensions.height, fps })).digest('hex').slice(0, 16);
  ...
  await run(ffmpegBin, ['-hide_banner', '-loglevel', 'error', '-y', '-ss', sourceStart.toFixed(6), '-t', duration.toFixed(6), '-i', sourcePath, '-map', '0:v:0', '-map', '0:a:0?', '-vf', `scale=${dimensions.width}:${dimensions.height}:flags=fast_bilinear,fps=${fps.toFixed(6)},format=yuv420p`, '-c:v', 'libx264', '-preset', 'ultrafast', '-crf', '27', '-c:a', 'aac', '-b:a', '96k', '-movflags', '+faststart', segmentPath], undefined, 300_000);
