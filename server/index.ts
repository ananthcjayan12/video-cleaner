...
app.post('/api/projects/:id/broll/export-video', route(async (req, res) => {
  ...
  const capabilities = await ffmpegCapabilities(settings.ffmpegBin); const encoding = encodingArgs(project, capabilities, mode, true); const inputAcceleration = capabilities.videoToolboxDecode ? ['-hwaccel', 'videotoolbox'] : []; const ranges = checkpointRanges(sourceSegments, activeScenes); if (!ranges.length) throw new Error('The export timeline is empty');
  ...
  void (async () => {
    try {
      ...
      const buildPartArgs = (range: CheckpointRange, partOutputPath: string) => {
        ...
        return ['-ss', localStart.toFixed(6), '-t', range.duration.toFixed(6), ...inputAcceleration, '-i', clip.sourcePath, ...brollInputs, ...presenterInput, '-filter_complex', filter, '-map', '[vout]', '-an', '-t', range.duration.toFixed(6), '-fps_mode', 'passthrough', ...encoding.args, ...bt709ColorArgs(), '-movflags', '+faststart', partOutputPath];
      };
