...
function App() {
  ...
  async function toggleWord(wordIndex: number) { if (!project || busy || exportRunning || generatingAll) return; const nextMask = [...keepMask]; nextMask[wordIndex] = !nextMask[wordIndex]; if (!nextMask.some(Boolean)) return; try { setEdl(await api.setEdl(project.id, maskToRanges(nextMask))); setStatus('Dialogue edit saved locally ✓'); } catch (err) { setError(message(err)); } }
