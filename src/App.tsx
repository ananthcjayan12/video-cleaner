...
function App() {
  ...
  const [activeStage, setActiveStage] = useState<WorkflowStage>('upload');
  ...
  async function changeBrollModelSetting(patch: Partial<Pick<BrollPlanSettings, 'provider' | 'imageModel' | 'planningProvider' | 'planningModel'>>) {
    const next = { ...brollSettings, ...patch };
    setBrollSettings(next);
    if (!project || !broll) return;
    try {
      setError('');
      const updated = await api.updateBrollSettings(project.id, patch);
      applyBrollPlan(updated);
    } catch (err) { setError(message(err)); setBrollSettings(broll.settings); }
  }

  async function changeThumbnailProvider(provider: ImageProvider) {
    setBrollSettings((current) => ({ ...current, provider }));
    if (!project) return;
    try {
      setError('');
      if (broll) {
        const updated = await api.updateBrollSettings(project.id, { provider });
        applyBrollPlan(updated);
      } else {
        await api.saveSettings({ imageProvider: provider });
        await refreshSystem();
      }
      setStatus(`Thumbnail image provider: ${providerLabel(provider)}. Saved locally ✓`);
    } catch (err) {
      setError(message(err));
      if (broll) setBrollSettings(broll.settings);
    }
  }
  ...
  async function pick() {
    await action('Opening native file picker…', async () => {
      ...
      const thumbnailState = await api.getThumbnail(selected.id); setThumbnail(thumbnailState); setThumbnailHook(thumbnailState.hook ?? ''); setActiveStage('upload'); setQuickEditorOpen(false);
  ...
  async function openProject(saved: Project) {
    await action(`Opening ${saved.name}…`, async () => {
      ...
      setActiveStage(suggestedStage(workflowFacts(snapshot.project, snapshot.transcript?.words?.length ?? 0, Boolean(snapshot.edl), snapshot.broll, thumbnailState)));
  ...
  async function closeProject() {
    clearAllAutosaves(); setProject(null); setProxyUrl(''); setWords([]); setEdl(null); resetBroll(); setThumbnail(null); setThumbnailHook(''); setActiveStage('upload'); setQuickEditorOpen(false); setExportJob(null); setError(''); await refreshProjects();
  }
  ...
  async function deleteProject(saved: Project) {
    ...
    await action('Deleting local project files…', async () => { await api.deleteProject(saved.id); if (project?.id === saved.id) { setProject(null); setProxyUrl(''); setWords([]); setEdl(null); resetBroll(); setThumbnail(null); setThumbnailHook(''); setActiveStage('upload'); setQuickEditorOpen(false); } await refreshProjects(); setStatus(`Deleted ${saved.name}. The original video was left untouched.`); });
  }
  ...
  async function exportLiveEditor(timeline: import('@cjcut/editor').CJCutProject) {
    if (!project || exportRunning || busy) return;
    await startExport('Saving CJCut timeline and choosing MP4 destination…', async () => {
      await api.saveEditorProject(project.id, timeline);
      return api.exportEditorVideo(project.id, 'quality');
    });
  }
  ...
  const ready = Boolean(
    system?.ffmpeg.installed && system?.ffprobe.installed && system?.elevenLabs.configured
    && textProviderConfigured(system, settingsForm.cleanProvider as TextProvider)
    && textProviderConfigured(system, settingsForm.planningProvider as TextProvider),
  );
  ...
  const stageFacts = project ? workflowFacts(project, words.length, Boolean(edl), broll, thumbnail) : null;
  const stageSteps = stageFacts ? workflowSteps(stageFacts) : [];
  const chooseStage = (next: WorkflowStage) => { setActiveStage(next); setQuickEditorOpen(false); setError(''); };
  const liveEditorProps = project ? {
    project, words, edl, broll, exportVideo: exportLiveEditor, exportJob,
    onStopExport: stopExport, exportBusy: busy || exportRunning,
  } : null;
  ...
  return (
    <main className="shell">
      ...
      {!project ? <ProjectLibrary projects={recentProjects} busy={busy} onNew={pick} onOpen={(saved) => void openProject(saved)} onRename={(saved) => void renameProject(saved)} onDelete={(saved) => void deleteProject(saved)} onRelink={(saved) => void relinkProject(saved)} onExport={(saved) => void exportEditableProject(saved)} onRefresh={() => void refreshProjects()} /> : <>
        ...
        <div className="creatorNextActions panel"><div><strong>Source ready?</strong><small>Cleaning is optional. You can edit raw footage immediately or plan B-roll directly.</small></div><button onClick={() => chooseStage('clean')}>Clean dialogue →</button><button onClick={() => chooseStage('story')}>Skip to B-roll plan</button><button className="primary" onClick={() => chooseStage('editor')} disabled={!project.sourceAvailable}>Edit now ↗</button></div>
        </> : <div className="creatorProjectStrip panel"><div><span className="label">CURRENT PROJECT</span><strong>{project.name}</strong><small>{sourceClips.length || 1} base clip(s) · {formatTime(project.media.duration)} · {project.media.width || '?'}×{project.media.height || '?'} · {project.sourceAvailable ? 'Source ready' : 'Source needs relink'}</small></div><button onClick={() => chooseStage('upload')}>Manage source clips</button></div>}
        ...
        <section className="workflow"><aside className="panel controls"><h3>Dialogue flow</h3><button onClick={prepare} disabled={busy || exportRunning || !!proxyUrl || !system?.ffmpeg.installed || !project.sourceAvailable}>1. Create proxy + audio</button><button onClick={transcribe} disabled={busy || exportRunning || !system?.elevenLabs.configured || !!words.length}>2. Transcribe audio</button><label>Cleanup intensity<select value={intensity} onChange={(e) => setIntensity(e.target.value as typeof intensity)}><option value="light">Light</option><option value="balanced">Balanced</option><option value="aggressive">Aggressive</option></select></label><ModelPicker title="Cleaning AI" provider={settingsForm.cleanProvider as TextProvider} model={settingsForm.cleanModel} system={system} agyModels={agyModels} onProvider={(value) => setSettingsForm((current) => ({ ...current, cleanProvider: value, cleanModel: '' }))} onModel={(value) => setSettingsForm((current) => ({ ...current, cleanModel: value }))} refreshAgy={() => void refreshAgyModels()} busy={busy || exportRunning} /><button onClick={clean} disabled={busy || exportRunning || !textProviderConfigured(system, settingsForm.cleanProvider as TextProvider)}>3. Clean with selected model</button><div className="divider" /><button onClick={() => exportBaseVideo('quality')} disabled={busy || exportRunning || !edl || !project.sourceAvailable}>Export cleaned video only</button>{exportJob && exportJob.state !== 'idle' && <ExportProgress job={exportJob} onStop={stopExport} />}</aside>
        ...
        {(activeStage === 'story' || activeStage === 'assets' || activeStage === 'thumbnail') &&         <BrollWorkspace focus={activeStage === 'thumbnail' ? 'thumbnail' : activeStage === 'story' ? 'story' : 'assets'} project={project} system={system} plan={broll} settings={brollSettings} setSettings={setBrollSettings} changeBrollModelSetting={changeBrollModelSetting} agyModels={agyModels} refreshAgyModels={refreshAgyModels} changeThumbnailProvider={changeThumbnailProvider} drafts={sceneDrafts} changeDraft={changeDraft} changeSceneLayout={changeSceneLayout} changeSceneAspect={changeSceneAspect} changeVideoProvider={changeVideoProvider} changeVideoAudio={changeVideoAudio} previewScene={previewScene} previewingScene={previewingScene} planBroll={planBroll} generateScene={generateScene} importSceneImage={importSceneImage} importSceneVideo={importSceneVideo} deleteScene={deleteScene} rewriteVideoPrompt={rewriteVideoPrompt} createSceneVideo={createSceneVideo} generateAll={generateAllBroll} generateMissing={generateMissingBroll} regenerateSelected={regenerateSelectedBroll} generateAllVideoPrompts={generateAllVideoPrompts} generateAllVideos={generateAllVideos} generateMissingVideos={generateMissingVideos} saveScene={saveScene} generating={brollGenerating} layoutSaving={layoutSaving} parallelRunning={parallelRunning} generatingAll={generatingAll} busy={busy || exportRunning} exportAssets={exportAssets} exportVideo={() => exportBrollVideo('quality')} imageConcurrency={imageConcurrency} setImageConcurrency={setImageConcurrency} videoConcurrency={videoConcurrency} setVideoConcurrency={setVideoConcurrency} flowUnmatched={flowUnmatched} syncGoogleFlow={syncGoogleFlow} assignGoogleFlowVideo={assignGoogleFlowVideo} thumbnail={thumbnail} thumbnailHook={thumbnailHook} setThumbnailHook={setThumbnailHook} thumbnailBusy={thumbnailBusy} selectThumbnailReferences={selectThumbnailReferences} generateThumbnail={generateThumbnail} />}
        {activeStage === 'story' && <div className="creatorNextActions panel"><div><strong>Make your scenes visible</strong><small>Generate images for selected story beats. The editor still works without B-roll.</small></div><button className="primary" onClick={() => chooseStage('assets')}>Create B-roll assets →</button><button onClick={() => chooseStage('editor')}>Edit without B-roll</button></div>}
        {activeStage === 'assets' && <div className="creatorNextActions panel"><div><strong>Use what is ready</strong><small>Images work as B-roll even if videos are missing; return later to generate motion.</small></div><button className="primary" onClick={() => chooseStage('editor')}>Edit with current assets →</button><button onClick={() => chooseStage('thumbnail')}>Create optional thumbnail</button></div>}
        {activeStage === 'thumbnail' && <div className="creatorNextActions panel"><div><strong>Thumbnail is optional</strong><small>Continue to export with or without a finished cover.</small></div><button className="primary" onClick={() => chooseStage('export')}>Go to export →</button><button onClick={() => chooseStage('editor')}>Continue editing</button></div>}
        {activeStage === 'editor' && liveEditorProps && <LiveEditor {...liveEditorProps} />}
        {activeStage === 'editor' && <div className="creatorNextActions panel"><div><strong>Video ready to render?</strong><small>Your current CJCut timeline is the final MP4 source. You can revisit any earlier step later.</small></div><button className="primary" onClick={() => chooseStage('export')}>Export options →</button><button onClick={() => chooseStage('assets')}>Return to B-roll</button></div>}
        {activeStage === 'export' && <section className="creatorExportStage">
          <div className="panel creatorExportHero"><div><span className="label">FINAL VIDEO · EDITOR-FIRST</span><h3>Export exactly what you edited</h3><p>Open the CJCut timeline, preview the current layers, and select “Export edited MP4”. Your trims, splits, layer visibility and B-roll positioning are used by the final renderer. You can also export without B-roll or a thumbnail.</p><button className="primary" onClick={() => chooseStage('editor')} disabled={!project.sourceAvailable}>Open editor & export MP4 ↗</button></div><div className="creatorExportFacts"><span>{stageFacts?.clipCount ?? 0} base clip(s)</span><span>{stageFacts?.imagesReady ?? 0} B-roll images</span><span>{stageFacts?.videosReady ?? 0} B-roll videos</span><span>{thumbnail?.hasImage ? 'Thumbnail ready' : 'Thumbnail optional'}</span></div></div>
          <div className="creatorExportOptions">
            <div className="panel"><h3>Cleaned / source-only MP4</h3><p>Use the original pipeline export if you do not need CJCut layers. Cleaning is optional.</p><button onClick={() => exportBaseVideo('quality')} disabled={busy || exportRunning || !project.sourceAvailable}>Export base video</button></div>
            <div className="panel"><h3>Editable project ZIP</h3><p>Download source clips, B-roll, timestamps, captions and selected assets.</p><button onClick={() => exportEditableProject()} disabled={busy || generatingAll}>Choose ZIP contents</button></div>
            <div className="panel"><h3>B-roll-only assets</h3><p>Export any available assets independently of the completed video.</p><button onClick={exportAssets} disabled={busy || generatingAll || !broll}>Export available B-roll</button></div>
            <div className="panel"><h3>Thumbnail</h3><p>{thumbnail?.hasImage ? 'Your branded cover is ready.' : 'A thumbnail is not required to export video.'}</p>{thumbnail?.hasImage ? <img className="creatorExportThumbnail" src={api.thumbnailImageUrl(project.id, thumbnail.generatedAt)} alt="Generated thumbnail" /> : <button onClick={() => chooseStage('thumbnail')}>Create optional cover</button>}</div>
          </div>
          {exportJob && exportJob.state !== 'idle' && <ExportProgress job={exportJob} onStop={stopExport} />}
        </section>}
        {quickEditorOpen && activeStage !== 'editor' && liveEditorProps && <div className="creatorEditorOverlay" role="dialog" aria-modal="true" aria-label="Quick live editor"><div className="creatorEditorOverlayBar"><div><strong>Live Editor</strong><small>Changes are saved locally. Close to return to {stageSteps.find((step) => step.id === activeStage)?.label || 'your workflow'}.</small></div><div><button onClick={() => { setQuickEditorOpen(false); setActiveStage('editor'); }}>Open full editor</button><button className="primary" onClick={() => setQuickEditorOpen(false)}>Back to workflow ✕</button></div></div><LiveEditor {...liveEditorProps} /></div>}
...
function SettingsPanel({ system, form, setForm, save, refresh, agyModels, refreshAgyModels, disabled, ffmpegDetail }: any) {
  return <section className="panel settingsPanel">
    <div className="modelSettingsGrid"><ModelPicker title="Cleaning model" provider={form.cleanProvider} model={form.cleanModel} system={system} agyModels={agyModels} onProvider={(value) => setForm((current: any) => ({ ...current, cleanProvider: value, cleanModel: '' }))} onModel={(value) => setForm((current: any) => ({ ...current, cleanModel: value }))} refreshAgy={refreshAgyModels} busy={disabled} /><ModelPicker title="Default B-roll planning + animation model" provider={form.planningProvider} model={form.planningModel} system={system} agyModels={agyModels} onProvider={(value) => setForm((current: any) => ({ ...current, planningProvider: value, planningModel: '' }))} onModel={(value) => setForm((current: any) => ({ ...current, planningModel: value }))} refreshAgy={refreshAgyModels} busy={disabled} /></div>
...
function BrollWorkspace({ focus = 'assets', project, system, plan, settings, setSettings, changeBrollModelSetting, agyModels, refreshAgyModels, changeThumbnailProvider, drafts, changeDraft, changeSceneLayout, changeSceneAspect, changeVideoProvider, changeVideoAudio, previewScene, previewingScene, planBroll, generateScene, importSceneImage, importSceneVideo, deleteScene, rewriteVideoPrompt, createSceneVideo, generateAll, generateMissing, regenerateSelected, generateAllVideoPrompts, generateAllVideos, generateMissingVideos, saveScene, generating, layoutSaving, parallelRunning, generatingAll, busy, exportAssets, exportVideo, imageConcurrency, setImageConcurrency, videoConcurrency, setVideoConcurrency, flowUnmatched, syncGoogleFlow, assignGoogleFlowVideo, thumbnail, thumbnailHook, setThumbnailHook, thumbnailBusy, selectThumbnailReferences, generateThumbnail }: any) {
  ...
  return <section className="panel brollPanel" data-focus={focus}>
    <div className="sectionTitle brollTitle"><div><span className="label">{focus === 'story' ? 'STORY FIRST · REVIEW BEFORE GENERATING' : focus === 'thumbnail' ? 'BRAND KIT · OPTIONAL COVER' : 'ASSET STUDIO · STILL IMAGES & MOTION'}</span><h3>{focus === 'story' ? 'Plan the visual story' : focus === 'thumbnail' ? 'Create your thumbnail' : 'Generate B-roll assets'}</h3></div>
    <div className="brollActions">
    {focus === 'story' && <button className="primary" onClick={planBroll} disabled={busy || generatingAll || !textProviderConfigured(system, settings.planningProvider || 'codex-cli') || !system?.elevenLabs.configured}>{plan ? 'Re-plan story beats' : 'Plan story beats'}</button>}
    {focus === 'assets' && <>{!plan ? <button onClick={planBroll} disabled={busy || generatingAll || !textProviderConfigured(system, settings.planningProvider || 'codex-cli') || !system?.elevenLabs.configured}>Plan scenes first</button> : <>
    {missingImages > 0 && <button className="primary" onClick={generateMissing} disabled={busy || generatingAll || !!generating || !providerIsReady}>Generate missing images ({missingImages})</button>}
    <button onClick={() => void regenerateCheckedScenes()} disabled={busy || generatingAll || !!generating || !providerIsReady || selectedScenes.length === 0}>Regenerate selected ({selectedScenes.length})</button>
    <button onClick={generateAll} disabled={busy || generatingAll || !!generating || !providerIsReady}>Regenerate all images</button>
    {imageScenes > 0 && <button onClick={generateAllVideoPrompts} disabled={busy || generatingAll || !!generating || !textProviderConfigured(system, settings.planningProvider || 'codex-cli')}>Generate motion prompts ({imageScenes})</button>}
    {missingVideos > 0 && <button onClick={generateMissingVideos} disabled={busy || generatingAll || !!generating || !videoProviderIsReady}>Generate missing videos ({missingVideos})</button>}
    <button onClick={generateAllVideos} disabled={busy || generatingAll || !!generating || !videoProviderIsReady || !plan.scenes.some((scene: BrollScene) => scene.imageFile)}>Regenerate all videos</button></>}</>}
    </div></div>
    {focus === 'story' && <ModelPicker title="Story, visual and motion planning AI" provider={settings.planningProvider || 'codex-cli'} model={settings.planningModel || ''} system={system} agyModels={agyModels} busy={busy || generatingAll || !!generating} refreshAgy={() => void refreshAgyModels()} onProvider={(provider) => void changeBrollModelSetting({ planningProvider: provider, planningModel: '' })} onModel={(model) => void changeBrollModelSetting({ planningModel: model })} />}
    {focus === 'assets' && <div className="modelSettingsGrid"><label>Image-generation provider<select value={settings.provider} onChange={(event) => void changeBrollModelSetting({ provider: event.target.value as ImageProvider, imageModel: '' })} disabled={busy || generatingAll || !!generating}><option value="gemini">Gemini Images API</option><option value="openai">OpenAI Images API</option><option value="codex-cli">Codex CLI · imagegen</option><option value="grok-cli">Grok CLI · experimental</option></select></label><label>Image model<input value={settings.imageModel || ''} onChange={(event) => void changeBrollModelSetting({ imageModel: event.target.value })} placeholder={settings.provider === 'gemini' ? system?.imageProviders?.gemini?.model || 'Gemini image default' : settings.provider === 'openai' ? system?.imageProviders?.openai?.model || 'OpenAI image default' : settings.provider === 'codex-cli' ? 'Codex CLI default' : system?.grok?.model || 'Grok CLI default'} disabled={busy || generatingAll || !!generating} /><small>Leave blank to use the configured provider default. This choice is independent of planning and cleaning.</small></label></div>}
    {focus !== 'thumbnail' && <div className="brollConfig"><label>Workflow<select value={settings.workflowMode} onChange={(e) => setSettings({ ...settings, workflowMode: e.target.value })}><option value="cleaned-video">Proxy/audio cleaned → B-roll → final video</option><option value="raw-video">Raw video → B-roll → final video</option><option value="assets-only">Raw video → B-roll files + timing JSON</option></select></label><label>Video provider<select value={videoProvider} onChange={(e) => void changeVideoProvider(e.target.value as VideoProvider)} disabled={busy || generatingAll || !!generating}><option value="magnific">Magnific · MiniMax Hailuo 2.3 Fast</option><option value="grok-cli">Grok CLI</option><option value="google-flow">Google Flow · Veo</option></select></label><label className="configToggle"><input type="checkbox" checked={Boolean(settings.returnVideoWithAudio)} onChange={(e) => void changeVideoAudio(e.target.checked)} disabled={busy || generatingAll || !!generating} /><span>Return video with audio<small>Off sends a silent prompt and removes the audio track after download.</small></span></label><label>Parallel images<select value={imageConcurrencyValue} onChange={(e) => setImageConcurrency(Number(e.target.value))}><option value="0">Auto ({imageProfile.defaultConcurrency})</option>{Array.from({ length: imageProfile.maxConcurrency }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1}</option>)}</select></label><label>Parallel videos<select value={videoConcurrencyValue} onChange={(e) => setVideoConcurrency(Number(e.target.value))}><option value="0">Auto ({videoProfile.defaultConcurrency})</option>{Array.from({ length: videoProfile.maxConcurrency }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1}</option>)}</select></label><label>B-roll frequency<select value={settings.countMode} onChange={(e) => setSettings({ ...settings, countMode: e.target.value })}><option value="auto">Auto by semantic scenes</option><option value="interval">One every N seconds</option><option value="exact">Exact count</option><option value="per-minute">B-rolls per minute</option></select></label>{settings.countMode === 'interval' && <label>B-roll interval<select value={settings.intervalSeconds} onChange={(e) => setSettings({ ...settings, intervalSeconds: Number(e.target.value) })}>{[10, 15, 20, 30, 45, 60, 90, 120].map((seconds) => <option key={seconds} value={seconds}>Every {seconds} seconds</option>)}</select></label>}{settings.countMode === 'exact' && <label>Exact B-rolls<input type="number" min="1" max="40" value={settings.targetCount} onChange={(e) => setSettings({ ...settings, targetCount: Number(e.target.value) })} /></label>}{settings.countMode === 'per-minute' && <label>B-rolls / minute<input type="number" min="0.5" max="20" step="0.5" value={settings.imagesPerMinute} onChange={(e) => setSettings({ ...settings, imagesPerMinute: Number(e.target.value) })} /></label>}<label>Aspect ratio<select value={settings.aspectRatio} onChange={(e) => setSettings({ ...settings, aspectRatio: e.target.value })}><option value="auto">Auto from video</option><option value="9:16">9:16 portrait</option><option value="16:9">16:9 landscape</option></select></label><label>Min scene sec<input type="number" min="1" max="20" value={settings.minSceneDuration} onChange={(e) => setSettings({ ...settings, minSceneDuration: Number(e.target.value) })} /></label><label>Max scene sec<input type="number" min="2" max="30" value={settings.maxSceneDuration} onChange={(e) => setSettings({ ...settings, maxSceneDuration: Number(e.target.value) })} /></label></div>}
    {focus === 'story' && <p className="muted brollModeHelp">The planner always leaves at least 5 seconds of talking-head footage between B-roll scenes. In interval mode it plans semantic scenes near the selected cadence. Scene timing and prompts autosave locally; interrupted generation can be resumed without regenerating completed scenes.</p>}

    {focus === 'thumbnail' && <div className="thumbnailPanel">
      ...
      <label className="thumbnailHook">Image provider for this thumbnail<select value={plan?.settings.provider ?? settings.provider} onChange={(event) => void changeThumbnailProvider(event.target.value)} disabled={busy || thumbnailBusy}><option value="gemini">Gemini API · reference images</option><option value="codex-cli">Codex CLI · reference images</option></select></label>
    ...
    {focus === 'assets' && plan && videoProvider === 'google-flow' && <div className="flowProjectPanel">
    ...
    {focus !== 'thumbnail' && (!plan ? <div className="brollEmpty"><strong>No B-roll plan yet</strong><p>Choose the workflow and image count, then plan scenes. Codex creates semantic scene boundaries, timestamps and hyper-real prompts.</p></div> : <>
      ...
      {focus === 'assets' && <div className="brollFooterActions"><button onClick={exportAssets} disabled={busy || generatingAll}>Export available images/videos + timing JSON</button><span className="muted">Your final MP4 is exported from Live Editor, preserving your manual edits.</span></div>}
    </>)}
