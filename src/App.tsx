...
function App() {
  ...
  async function saveManualEdl(keepRanges: KeepRange[]) {
    if (!project) return;
    const updated = await api.setEdl(project.id, keepRanges); setEdl(updated); resetBroll();
  ...
  return (
    <main className="shell">
      ...
      {!project ? <ProjectLibrary projects={recentProjects} busy={busy} onNew={pick} onOpen={(saved) => void openProject(saved)} onRename={(saved) => void renameProject(saved)} onDelete={(saved) => void deleteProject(saved)} onRelink={(saved) => void relinkProject(saved)} onRefresh={() => void refreshProjects()} /> : <>
        ...
        {proxyUrl && edl && words.length > 0 && <TimelineEditorPanel duration={project.media.duration} fps={project.media.frameRate} words={words} edl={edl} videoRef={videoRef} disabled={busy || exportRunning || generatingAll} onSave={saveManualEdl} onNotice={setStatus} />}
