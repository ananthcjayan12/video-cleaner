...
function App() {
  ...
  function resetBroll() { clearAllAutosaves(); setBroll(null); setSceneDrafts({}); draftRef.current = {}; setBrollGenerating(null); setPreviewingScene(null); setScenePreview(null); setParallelRunning([]); }
  ...
  async function toggleWord(wordIndex: number) { if (!project || busy || exportRunning || generatingAll) return; const nextMask = [...keepMask]; nextMask[wordIndex] = !nextMask[wordIndex]; if (!nextMask.some(Boolean)) return; try { setEdl(await api.setEdl(project.id, maskToRanges(nextMask))); resetBroll(); setStatus('Dialogue edit saved locally ✓'); } catch (err) { setError(message(err)); } }
  ...
  async function createSceneVideo(scene: BrollScene, regenerationComment?: string) {
    ...
    try { setError(''); setBrollGenerating(scene.id); setStatus(`${scene.videoPrompt ? 'Creating' : 'Codex is prompting, then creating'} video for ${scene.title} with Grok CLI…`); const saved = await saveScene(scene); const result = await api.createBrollVideo(project.id, saved.id, regenerationComment?.trim() || undefined); replaceScene(result.scene); setStatus(`B-roll video ready for ${result.scene.title}. Saved locally ✓`); }
    catch (err) { setError(message(err)); } finally { setBrollGenerating(null); }
  }
  ...
  async function runImageBatch(scenes: BrollScene[], mode: 'all' | 'missing') {
    if (!project || !broll || generatingAll || brollGenerating) return;
    if (!scenes.length) { setStatus('All B-roll images are already present.'); return; }
    ...
    if (!window.confirm(`${mode === 'missing' ? 'Generate the' : 'Generate'} ${scenes.length} ${mode === 'missing' ? 'missing ' : ''}B-roll image${scenes.length === 1 ? '' : 's'} using ${provider} with up to ${concurrency} parallel worker${concurrency === 1 ? '' : 's'}?`)) return;
  ...
  async function runVideoBatch(scenes: BrollScene[], mode: 'all' | 'missing') {
    ...
    const concurrency = resolveConcurrency(videoConcurrency, 2, 3);
    if (!window.confirm(`${mode === 'missing' ? 'Create the' : 'Create'} ${scenes.length} ${mode === 'missing' ? 'missing ' : ''}B-roll video${scenes.length === 1 ? '' : 's'} with Grok CLI using up to ${concurrency} parallel worker${concurrency === 1 ? '' : 's'}?`)) return;
    setGeneratingAll(true); setParallelRunning([]); setError('');
    try {
      const savedScenes: BrollScene[] = []; for (const scene of scenes) savedScenes.push(await saveScene(scene));
      const failures = await runParallel(savedScenes, concurrency, async (scene) => {
      ...
      await finalizeParallelPlan(savedScenes); const succeeded = savedScenes.length - failures.length;
      setStatus(`Parallel video generation complete: ${succeeded}/${savedScenes.length} succeeded. Project saved locally ✓`);
    ...
    <main className="shell">
      ...
      {!project ? <ProjectLibrary projects={recentProjects} busy={busy} onNew={pick} onOpen={(saved) => void openProject(saved)} onRename={(saved) => void renameProject(saved)} onDelete={(saved) => void deleteProject(saved)} onRelink={(saved) => void relinkProject(saved)} onRefresh={() => void refreshProjects()} /> : <>
        <section className={`source panel ${project.sourceAvailable ? '' : 'missingSourcePanel'}`}>
          <div><span className="label">{project.sourceAvailable ? 'MASTER SOURCE' : 'MASTER SOURCE MISSING'}</span><strong>{project.name}</strong><small>{project.sourceName} · Original media stays referenced in place.</small></div>
          <div className="sourceRight"><div className="chips"><span>{project.media.width}×{project.media.height}</span><span>{String(project.media.videoCodec).toUpperCase()}</span>{project.media.frameRate ? <span>{project.media.frameRate.toFixed(2)} fps</span> : null}<span>{(project.media.size / 1024 / 1024 / 1024).toFixed(2)} GB</span>{project.media.hdr && <span className="warn">HDR</span>}</div>{!project.sourceAvailable && <button className="danger" onClick={() => void relinkProject(project)} disabled={busy || generatingAll}>Relink original video</button>}</div>
        </section>
...
function BrollWorkspace({ project, system, plan, settings, setSettings, drafts, changeDraft, changeSceneLayout, previewScene, previewingScene, planBroll, generateScene, importSceneImage, deleteScene, rewriteVideoPrompt, createSceneVideo, generateAll, generateMissing, generateAllVideoPrompts, generateAllVideos, generateMissingVideos, saveScene, generating, layoutSaving, parallelRunning, generatingAll, busy, exportAssets, exportVideo, imageConcurrency, setImageConcurrency, videoConcurrency, setVideoConcurrency }: any) {
  ...
  return <section className="panel brollPanel">
    ...
    <p className="muted brollModeHelp">Scene title, timing and prompts autosave locally about 700 ms after editing. Interrupted generation can be resumed with Generate missing images / Create missing videos without spending again on completed scenes.</p>
...
function ExportProgress({ job, onStop }: { job: ExportStatus; onStop: () => void }) { const progress = Math.max(0, Math.min(100, job.progress || 0)); const title = job.state === 'completed' ? 'Export complete' : job.state === 'failed' ? 'Export failed' : job.state === 'stopped' ? 'Export paused' : job.resumed ? 'Resuming master' : 'Rendering master'; return <div className={`exportProgress ${job.state}`}><div className="exportProgressHead"><strong>{title}</strong><span>{progress.toFixed(1)}%</span></div><div className="progressTrack"><span style={{ width: `${progress}%` }} /></div><div className="exportMeta"><span>{job.encoder || 'FFmpeg'}</span>{job.speed && <span>{job.speed}</span>}{job.checkpointTotal ? <span>{job.checkpointCompleted || 0}/{job.checkpointTotal} checkpoints</span> : null}{job.frame > 0 && <span>{job.frame.toLocaleString()} frames</span>}</div>{job.state === 'running' && <button className="danger exportStop" onClick={onStop}>{job.resumable ? 'Stop and keep checkpoints' : 'Stop rendering'}</button>}{job.state === 'stopped' && job.resumable && <small>Choose Render final video again to resume.</small>}</div>; }
