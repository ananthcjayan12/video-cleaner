...
function BrollWorkspace({ focus = 'assets', project, system, plan, settings, setSettings, changeThumbnailProvider, drafts, changeDraft, changeSceneLayout, changeSceneAspect, changeVideoProvider, changeVideoAudio, previewScene, previewingScene, planBroll, generateScene, importSceneImage, importSceneVideo, deleteScene, rewriteVideoPrompt, createSceneVideo, generateAll, generateMissing, regenerateSelected, generateAllVideoPrompts, generateAllVideos, generateMissingVideos, saveScene, generating, layoutSaving, parallelRunning, generatingAll, busy, exportAssets, exportVideo, imageConcurrency, setImageConcurrency, videoConcurrency, setVideoConcurrency, flowUnmatched, syncGoogleFlow, assignGoogleFlowVideo, thumbnail, thumbnailHook, setThumbnailHook, thumbnailBusy, selectThumbnailReferences, generateThumbnail }: any) {
  ...
  return <section className="panel brollPanel" data-focus={focus}>
    ...
    {focus === 'story' && <button className="primary" onClick={planBroll} disabled={busy || generatingAll || !system?.codex.authenticated || !system?.elevenLabs.configured}>{plan ? 'Re-plan story beats' : 'Plan story beats'}</button>}
    {focus === 'assets' && <>{!plan ? <button onClick={planBroll} disabled={busy || generatingAll || !system?.codex.authenticated || !system?.elevenLabs.configured}>Plan scenes first</button> : <>
    ...
    {imageScenes > 0 && <button onClick={generateAllVideoPrompts} disabled={busy || generatingAll || !!generating || !system?.codex.authenticated}>Generate motion prompts ({imageScenes})</button>}
