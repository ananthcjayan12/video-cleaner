...
function App() {
  ...
  async function refreshSystem() {
    try {
      ...
      setSettingsForm((current) => ({
        ...
        openAiImageModel: result.overrides?.openAiImageModel ?? '', geminiImageModel: result.overrides?.geminiImageModel ?? '', grokModel: result.overrides?.grokModel ?? '', grokVideoModel: result.overrides?.grokVideoModel ?? '', gflowProfile: result.overrides?.gflowProfile ?? '', gflowVideoModel: result.overrides?.gflowVideoModel ?? '', freepikApiKey: '', freepikVideoModel: result.overrides?.freepikVideoModel ?? '', freepikVideoEndpoint: result.overrides?.freepikVideoEndpoint ?? '',
      }));
  ...
  async function saveSettings() {
    await action('Saving local settings…', async () => {
      ...
      setSettingsForm((current) => ({ ...current, elevenLabsApiKey: '', openAiApiKey: '', geminiApiKey: '', freepikApiKey: '' }));
...
function SettingsPanel({ system, form, setForm, save, refresh, disabled, ffmpegDetail }: any) {
  return <section className="panel settingsPanel">
    ...
    <div className="settingsGrid">
      ...
      <label>Google Flow video model<input value={form.gflowVideoModel} onChange={(e) => setForm({ ...form, gflowVideoModel: e.target.value })} placeholder={system?.videoProviders?.googleFlow.model || 'veo-fast'} /></label><label>Magnific / Freepik API key<input type="password" value={form.freepikApiKey} onChange={(e) => setForm({ ...form, freepikApiKey: e.target.value })} placeholder={system?.videoProviders?.magnific.configured ? 'Configured — enter only to replace' : 'Freepik API key'} /></label><label>Magnific video model<input value={form.freepikVideoModel} onChange={(e) => setForm({ ...form, freepikVideoModel: e.target.value })} placeholder={system?.videoProviders?.magnific.model || 'minimax-h3-max-turbo'} /></label><label className="wide">Magnific endpoint override<input value={form.freepikVideoEndpoint} onChange={(e) => setForm({ ...form, freepikVideoEndpoint: e.target.value })} placeholder={system?.videoProviders?.magnific.endpoint || 'Auto from model'} /></label>
