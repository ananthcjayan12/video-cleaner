...
function App() {
  ...
  async function exportEditableProject(target?: Project) {
    const selected = target ?? project; if (!selected) return;
    await action('Choose destination for editable project ZIP…', async () => {
      const result = await api.exportProjectZip(selected.id);
      setStatus(`Editable project ZIP exported: ${result.destination} · ${result.clips} talking-head clip${result.clips === 1 ? '' : 's'} · ${result.brollImages} B-roll image${result.brollImages === 1 ? '' : 's'} · ${result.brollVideos} B-roll video${result.brollVideos === 1 ? '' : 's'}`);
    });
  }
