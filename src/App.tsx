      ...
      <div className="brollGrid">{plan.scenes.map((scene: BrollScene) => {
        ...
        return <article className={`brollCard ${draft.enabled ? '' : 'disabledScene'} ${selectedForBatch ? 'selectedForBatch' : ''}`} key={scene.id}>
          ...
          <div className="brollBody">
            ...
            <div className="sceneMeta"><span>{scene.id}</span>{scene.beatType && <span className="storyBadge">{scene.beatType}</span>}{scene.visualMode && <span className="modeBadge">{scene.visualMode}</span>}<span>{scene.shotType || 'shot'}</span>{scene.provider && <span>{providerLabel(scene.provider)}</span>}{isWorking && <span>WORKING</span>}{scene.videoFile && <span className="videoBadge">VIDEO</span>}{scene.videoProvider && <span>{videoProviderLabel(scene.videoProvider)}</span>}{scene.videoModel && <span>{scene.videoModel}</span>}</div>
